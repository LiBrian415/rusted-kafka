...
async fn spawn_multi_server_testers(start_port: usize, count: usize) -> (Vec<String>, Vec<ServerTester>) {
...
async fn test_tester_shutdown() -> Result<(), Box<(dyn Error + Send + Sync)>> {
    let (addrs, mut server_testers) = spawn_multi_server_testers(3000, 1).await;
