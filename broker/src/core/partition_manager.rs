...
impl PartitionManager {
    ...
    pub fn set_leader_partition(
        ...
        if let Some(partition_state) = (*w).get(&topic_partition) {
            // We can't replace since we should still maintain the the follower fetch and
            // offsets
...
impl PartitionState {
    ...
    pub fn check_and_update_version(&self, leader_and_isr: &LeaderAndIsr, update: bool) -> bool {
        ...
        // Consider invalid update if the request has any small epochs or if it's exactl the same
